...
void insert_termIO (pid, sockfd, outstr, type)
int pid, sockfd, type;
char *outstr;
{ TermQnode *node;
  sem_wait (&term_mutex);
...
{ TermQnode *node; 
  ...
  { node = termQhead;
      ...
      else
      {
        int ret;
	//printf("%s\n",node->str);
    	ret = send(node->sockfd, node->str , 200, 0);
...
void *termIO ()
{
  while (systemActive) 
  {
	sem_wait (&term_empty);
	sem_wait (&term_mutex); 
	if(!systemActive && termQtail == NULL)
		break;
	handle_one_termIO ();
	sem_post (&term_mutex);
  }  
...
{ int ret;
  ...
  sem_init( &term_empty, 0, 0 );
  pthread_create (&termThread, NULL, termIO, NULL);
  fprintf (infF, "Terminal Manager thread has been created succesfully\n");  
...
{ 
  ...
  sem_close(&term_mutex);
  sem_close(&term_empty);
  fclose (fterm);
